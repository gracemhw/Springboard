...
SELECT DISTINCT CONCAT_WS(' ', m.firstname, m.surname) AS fullname, f.name AS courtname
FROM Bookings AS b
...
SELECT f.facid, f.name, SUM(b.slots) AS slots
FROM Bookings as b
LEFT JOIN Facilities as f
ON b.facid = f.facid
WHERE b.memid != 0
...
SELECT facility_name, month, SUM(slots) AS slots
FROM (
